...
| `channel` | string | No | One of `web`, `cli`, `telegram`, `whatsapp`, `discord`, `automation`, `task`, `subagent`. Defaults to `web`, the same default `GET /v1/sessions` uses |

**Returns:** `{ sessions }`, the same summary shape the sessions API returns.

**Limits:**

- Same organization only — a profile in another organization returns the same "Profile not found." error as an id that does not exist, so the tool never confirms an id exists elsewhere
...
- Same organization only — a session outside it returns "Session not found."
