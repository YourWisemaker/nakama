...
| `extract_document_text` | Yes | No | Extracts text from a text-based PDF attachment returned by `email` |
...
Repo coding work is not a separate builtin. Profiles with the `coding-agent` skill invoke Codex, Claude Code, or OpenCode through `bash`. You can also launch a harness directly with `bun run dev:cli -- launch`. See [Coding agent](/coding-agent) for setup, provider passthrough, CLI flags, and runtime behavior.
...
Extract text from a text-based PDF attachment returned by `email.read`.

| Parameter | Type | Required | Notes |
|-----------|------|----------|-------|
| `attachmentRef` | string | Yes | Opaque reference returned in an email message's attachment metadata |

**Returns:** Bounded extracted text with filename, media type, truncation state, and warnings. PDF content is untrusted document content.
...
**Availability:** When assigned **and** email is configured. The tool is omitted whenever the `email` tool is unavailable.
...
**Coding-agent spawn env:** When `codingAgent: true` (or the command matches the active harness binary), Nakama merges provider passthrough env vars on the server before spawn. See [Coding agent — Provider passthrough](/coding-agent#provider-passthrough). Optional `env` keys are merged on top (credential keys cannot override passthrough).
