...
- **Archive (90 days unused):** the skill folder is **moved** to `skills/.archive/` on that profile. Nothing is deleted. The skill is unassigned so it leaves the chat catalog. An admin can restore it later from disk (restore UI is not in this release).
