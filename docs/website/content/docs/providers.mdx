...
- **Image parsing** — used when the chat model cannot see images. Set **Settings → Image parsing model**. Custom (OpenAI-compatible) models are included by default; turn **Vision** off on a model if it is text-only
