---
title: "Coding Agent"
description: "Launch Codex, Claude Code, or OpenCode from Nakama chat or the CLI, with optional provider passthrough from your Nakama LLM provider."
---
...
- Provider passthrough enabled in Integrations → Coding agents (default: on)
...
`nakama launch` always requests a launch plan from the server, which includes the same spawn env (full secrets only for local-token CLI auth).

**Compatibility:** Integrations → Coding agents shows whether passthrough is active for the selected harness. Incompatible provider/harness pairs show a warning with a link to Settings → Provider.

**Security:** Passthrough injects the real org provider API key into subprocess env on the host. Use on trusted single-tenant machines only. Any org member who can invoke coding agents receives provider credentials in the spawn environment.

**Disable passthrough:** Turn off provider passthrough in Integrations → Coding agents to fall back to vendor login on the host (`claude /login`, etc.).
...
### Verify provider passthrough

After launch, the CLI prints either:

- `Provider passthrough enabled (Anthropic / claude-sonnet-4-6)` — the harness should **not** ask for `/login`
- A warning that no passthrough env was applied — configure a compatible provider in Settings → Provider

Without passthrough, the coding CLI uses vendor authentication on the host (`claude /login`, Codex login, etc.).
...
Runs execute in the **active profile workspace** when delegated from chat (`~/.nakama/orgs/{orgId}/profiles/{profileId}/`). `nakama launch` defaults to your **current shell directory** unless you pass `--cwd`. When provider passthrough is active, spawn env routes API calls to your Nakama-configured provider using the selected profile's model; otherwise the coding CLI uses vendor auth configured on the host.
