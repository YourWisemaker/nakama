...
**Security:** Passthrough injects the real org provider API key into subprocess env on the host. Use on trusted single-tenant machines only.
...
| `--profile ID` | Profile for provider/model routing (aliases: `super_bot`, `super-bot`, `Super Bot`) |
...
- A warning that passthrough is not active — configure a compatible provider in Settings → Provider

Coding agents always use Nakama provider passthrough; harness-native vendor login (`claude /login`, etc.) is not supported yet.
